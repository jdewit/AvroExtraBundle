...
class ScriptHandler
{
    public static function postInstallSymlinkExtras(Event $event)
    {
        ...
        $rootDir = $extra['symfony-root-dir'];
        $appDir = $extra['symfony-app-dir'];
        $webDir = $extra['symfony-web-dir'];
        $assets = $extra['assets'];

        foreach($assets as $asset) {
            $originDir = $rootDir.'/vendor/'.$asset;
            $targetDir = $rootDir.'/web/assets/'.$asset;

            if (!function_exists('symlink')) {
                $IO->write("Symlink not supported on your machine: ", FALSE);
                $IO->write(" ... <error>Fail</error>");

                return;
            }

            $IO->write("Creating Symlink: " . $targetDir, FALSE);

            self::copyDir($originDir, $targetDir);
    ...
    {
        foreach (self::toIterator($dirs) as $dir) {
            if (is_dir($dir)) {
                continue;
            }
    ...
    {
        foreach (self::toIterator($files) as $file) {
            if (!file_exists($file)) {
                return false;
            }
        }
    ...
    {
        $files = iterator_to_array(self::toIterator($files));
    ...
    public static function mirror($originDir, $targetDir, \Traversable $iterator = null, $options = array())
    {
        ...
        foreach ($iterator as $file) {
            ...
            if (is_dir($file)) {
                ...
                self::copy($file, $target, isset($options['override']) ? $options['override'] : false);
            } else {
     ...
     * @return \Traversable
     */
    private static function toIterator($files)
    {
